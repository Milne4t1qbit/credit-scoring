...
def cornerize(x,y):
# plot flat lines at constant values 
# by adding a new x y point just before the next x
    xNew=[];yNew=[];nOriginals=min(len(x),len(y));tol=0.00001
    for i in range(nOriginals-1):
        xNew.append(x[i]);
        xNew.append(x[i+1]-tol)
        yNew.append(y[i])
        yNew.append(y[i])
    #endfor
    return xNew,yNew
